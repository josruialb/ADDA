...
//B) -------------------------------------------JAVA10----------------------------------------------------
	...
	public static Integer cuentaCaracteresJ10(String s) {
		return (int) Stream.iterate(0, x -> x + 1).limit(s.length()).filter(x -> Character.isLowerCase(s.charAt(x)))
				.count();

	}

	public static String java10(List<String> ls) {
		return ls.stream().max(Comparator.comparingInt(x -> cuentaCaracteresJ10(x))).get();
...
//C)-----------------------------------------RECURSIVO---------------------------------------------------
	...
	public static void main(String[] args) {
		...
		System.out.println("Recursivo: "+Ej1RecFinal(e1, "", 0));

		System.out.println("JAVA 10:   "+java10(e1));

		System.out.println("Iterativo: "+Iterativo1(e1));
